...
def test_get_snps_2():
    assert get_snps('test/resources/snps') == 'rs3890745\nrs2240340\nrs2476601'

def test_get_zoomed_genes_1():
    assert get_zoomed_genes(None) == ''
def test_get_zoomed_genes_2():
    assert get_zoomed_genes('test/resources/zoom_gene') == 'BRCA1,BRCA2'
@raises(SystemExit)
...
def test_get_specified_genes_2():
    assert get_specified_genes('test/resources/specified') == 'BRCA1\nBRCA2'
@raises(SystemExit)
...
def test_create_request_2():
    parser = create_parser()
    args = parser.parse_args(['-f', 'test/resourcessnps'])
    create_request(args)

@raises(SystemExit)
def test_create_request_3():
    parser = create_parser()
    args = parser.parse_args(['-f', 'test/resources/snps', '-d', 'description'])
    create_request(args)

class TestRequest(unittest.TestCase):
    def test_create_request_4(self):
        parser = create_parser()
        args = parser.parse_args(['-f', 'test/resources/snps', '-d', 'description', '-e', 'test@test'])
...
def fail_urlopen(url):
    url_file = open('test/resources/fail.html', 'rb')
    return url_file

def status_urlopen(url):
    url_file = open('test/resources/status.html', 'rb')
    return url_file
...
def finished_urlopen(url):
    url_file = open('test/resources/finished.html', 'rb')
    return url_file
...
def run_urlopen(url):
    url_file = open('test/resources/run.html', 'rb')
    return url_file
